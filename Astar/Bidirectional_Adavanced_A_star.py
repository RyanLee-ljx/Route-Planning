...
def f_value_cal(node, map):
    ...
    f_value = node.g_value + node.h_value * 1.001
...
def astar(start, final, map):
    ...
    final_node.e_value, start_node.f_value = f_value_cal(final_node, map)
    ...
    while open_list_backward and open_list_forward:
        ...
        if len(open_list_forward) <= len(open_list_backward):
            ...
            target_node = current_node_forward_list[0][1]
        ...
        for neighbour in neighbours:
            ...
            if n[0] < 0 or n[0] >= len(map[0]) or n[1] < 0 or n[1] >= len(map):
                ...
                next_node = Node(position=n, g=current_node.g_value + dist[abs(neighbour[0]) + abs(neighbour[1])]
                                 , h=heuristic_function(target_node.position, n), parent=current_node)
